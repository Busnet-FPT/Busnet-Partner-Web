...
function AddSchedulePage() {
  ...
  // (blurred) or once they've tried to advance past its step — so errors
  // never flash before the user has had a chance to type anything.
  ...
  // Every field's error, recomputed live — but only fields that are
  // "touched" (blurred, or the step containing them was already attempted)
  // actually render their message. Keyed by field name so both the Input's
  // red-border state and its <FieldError> can look themselves up by the
  // same key.
  const fieldErrors = useMemo(() => {
    ...
    else if (startDate === todayStr() && departureTime < nowTimeStr())
    ...
    else if (startDate < todayStr()) e.startDate = 'Cannot be in the past'
    ...
    return e
  }, [routeId, recurrenceType, daysOfWeek, daysOfMonth, departureTime, arrivalTime, arrivalDayOffset, basePrice, startDate, endDate, pickupPoints, dropoffPoints, busId])

  // Should this specific field's error actually be rendered right now?
  ...
  const handleSubmit = async () => {
    // Guard against a stale/tampered step 3 (Review) submit — re-check every step.
    ...
    <div className="motion-safe:animate-in fade-in mx-auto max-w-4xl duration-500">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" className="transition-all hover:-translate-y-0.5 active:translate-y-0" onClick={() => navigate('/schedules')}>
            Cancel
          </Button>
          <Button
      ...
      <div className="mt-6 flex items-center justify-between">
        <Button
          variant="outline"
          className="text-red-600 border-red-200 transition-all hover:-translate-y-0.5 hover:bg-red-50 active:translate-y-0"
